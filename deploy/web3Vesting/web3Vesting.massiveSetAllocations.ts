...
const func: DeployFunction = async (hre: HardhatRuntimeEnvironment): Promise<void> => {
  await callWithTimerHre(async () => {
    ...
    console.log(`${WEB3_VESTING_NAME} ${web3VestingAddress} is setting allocations in batches...`);
    const erc20TokenAddress = contractConfig.erc20Token;
    const users = await getUsers();
    const context = await getContext(erc20TokenAddress, web3VestingAddress);
    const { owner2WEB3Vesting } = context;
      ...
      try {
        ...
        await Promise.all(
          addresses.map(async (address) => {
            const claimInfo = await owner2WEB3Vesting.fetchClaimInfo(address);
          ...
          await retry({
            fn: async () => {
              if (SIMULATE) {
                ...
                await owner2WEB3Vesting.setAllocations(recipients, amounts, TX_OVERRIDES);
              }
            },
    ...
    const totalAllocatedInContract = await owner2WEB3Vesting.totalAllocated();
...
func.tags = [`${WEB3_VESTING_NAME}:massive-set-allocations`];
