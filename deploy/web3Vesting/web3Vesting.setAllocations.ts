...
const func: DeployFunction = async (hre: HardhatRuntimeEnvironment): Promise<void> => {
  await callWithTimerHre(async () => {
    ...
    console.log(`${WEB3_VESTING_NAME} ${web3VestingAddress} is setting allocations...`);
    const erc20TokenAddress = contractConfig.erc20Token;
    const context = await getContext(erc20TokenAddress, web3VestingAddress);
    const { user1Address, user2Address, user3Address, owner2WEB3Vesting, web3VestingFactory } =
      context;
    ...
    await waitTx(
      owner2WEB3Vesting.setAllocations(params.recipients, params.amounts, TX_OVERRIDES),
      'setAllocations',
...
func.tags = [`${WEB3_VESTING_NAME}:set-allocations`];
